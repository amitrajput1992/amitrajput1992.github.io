...
export function simplex3(xin, yin, zin) {
  ...
  const y0 = yin - j + t;
  const z0 = zin - k + t; // For the 3D case, the simplex shape is a slightly irregular tetrahedron.
  ...
  const y3 = y0 - 1 + 3 * G3;
  const z3 = z0 - 1 + 3 * G3; // Work out the hashed gradient indices of the four simplex corners
  ...
  const gi3 = gradP[i + 1 + perm[j + 1 + perm[k + 1]]]; // Calculate the contribution from the four corners
